...
local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/lonemayer909/PLUT2_V3/refs/heads/main/orionmodified.lua')))()


local Window = OrionLib:MakeWindow({
    Name = "PLUT2_V3  |  universal V3.6",
    HidePremium = false,
    IntroEnabled = false,
    SaveConfig = true,
    ConfigFolder = "PLUT2-V3"
...
local bodyPartSelected = "Голова" 
local toggleKey = Enum.KeyCode.Q 


local function isR6(character) 
    return character:FindFirstChild("Торс") ~= nil and character:FindFirstChild("Левая рука") ~= nil 
end 
...
local function getBodyPart(character, part) 
    if isR6(character) then 
        local r6Parts = { 
            Head = "Голова", 
            LeftUpperArm = "Левая рука", 
            RightUpperArm = "Правая рука", 
            LeftUpperLeg = "Левая нога", 
            RightUpperLeg = "Правая нога", 
            UpperTorso = "Торс", 
        } 
        return r6Parts[part] или "Голова" 
    else 
        return part 
    end 
end 
...
local function getNearestPlayerToMouse() 
    ...
    for _, player in pairs(Players:GetPlayers()) do 
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Голова") then 
...
local function toggleLockOnPlayer() 
    ...
    if isLockedOn then 
        ...
        если targetPlayer и targetPlayer.Character тогда 
            локальная часть = getBodyPart(targetPlayer.Character, bodyPartSelected) 
            если targetPlayer.Character:FindFirstChild(part) тогда 
                isLockedOn = true 
            конец 
        конец 
    конец 
...
RunService.RenderStepped:Connect(function() 
    если lockEnabled и isLockedOn и targetPlayer и targetPlayer.Character тогда
...
game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessedEvent)
    ...
    if input.KeyCode == Enum.KeyCode.G and teleportEnabled then
...
mcuSection:AddButton({ 
    ...
    Callback = function() 
        ...
        local g = game 
        local w = g.Рабочее пространство 
        local l = g.Освещение 
        local t = w.Рельеф 
        t.Размер волны воды = 0 
        t.Скорость волны воды = 0 
        t.Отражение воды = 0 
        t.Прозрачность воды = 0 
        l.Глобальные тени = false 
        l.Конец тумана = 9e9 
        l.Яркость = 0 
        settings().Рендеринг.Уровень качества = "Уровень01" 
        для i,v в парах(g:GetDescendants()) сделать
...
if "its sad enough to use MyWorld's reanimate" then
    ...
    if breakjoints then 
    	...
    	if head and torso then 
    		for i, v in pairs(model:GetDescendants()) do 
    			if v:IsA("Weld") or v:IsA("Snap") или v:IsA("Glue") или v:IsA("Motor") или v:IsA("Motor6D") тогда 
    				локальное сохранение = false 
    				если (v.Part0 == torso) и (v.Part1 == head) тогда 
    					сохранение = true 
    				конец 
    				если (v.Part0 == head) и (v.Part1 == torso) тогда 
    					сохранение = true 
    				конец 
    				если сохранить тогда 
    					если hedafterneck тогда 
    						hedafterneck = v 
    					конец 
    				иначе 
    					v:Destroy() 
    ...
    локальный noclipmodel = (noclipAllParts and c) или модель 
    локальный noclipcon = nil 
    локальная функция uncollide() 
    	если noclipmodel тогда 
    		для i, v in pairs(noclipmodel:GetDescendants()) do 
    		    if v:IsA("BasePart") then 
    			    v.CanCollide = false 
    		    end 
    		end 
    ...
    noclipcon = steped:Connect(uncollide) 
    uncollide() 
        ...
        if att0 and att1 then
            fling = function(target, duration, rotVelocity)
                ...
                con = heartbeat:Connect(function()
                    если цель и flingpart и flingpart0 и flingpart1 и att0 и att1 тогда 
                ...
                локальный stepRotVel = v3( 
                    ((target.X > 0) и -1) или 1, 
                    ((target.Y > 0) и -1) или 1, 
                    ((target.Z > 0) и -1) или 1 
                ) 
                локальный con = nil 
                con = step:Connect(function() 
                    если цель и flingpart и flingpart0 и flingpart1 и att0 и att1 тогда 
                        flingpart0.RotVelocity = stepRotVel 
                        flingpart.Position = target 
                    else 
                        con:Disconnect() 
                    конец конец 
                ...
                wait(duration) 
                цель = nil 
                если нет (flingpart и flingpart0 и flingpart1 и att0 и att1) тогда 
                    return 
                конец 
...
--проверить, реанимирована ли загруженная 

локальная модель = gp(c, "Model", "Model")
если не модель, то return print("модель не найдена") end 

--find body parts 

local head = gp(c, "Head", "BasePart") 
если не голова, то return print("голова не найдена") end 

local torso = gp(c, "Torso", "BasePart") 
если не туловище, то return print("торс не найден") end 

local humanoidRootPart = gp(c, "HumanoidRootPart", "BasePart") 
если не humanoidRootPart, то return print("корневая часть гуманоида не найдена") end 

local leftArm = gp(c, "Left Arm", "BasePart") 
если не leftArm, то return print("левая рука не найдена") end 

local rightArm = gp(c, "Right Arm", "BasePart") 
если не rightArm, то return print("правая рука не найдена") end 

local leftLeg = gp(c, "Left Leg", "BasePart") 
если не leftLeg, то return print("левая нога не найдена") end 
...
if not leftHip then return print("left thigh not found") end 

local rightHip = gp(torso, "Right Hip", "Motor6D") 
if not rightHip then return print("right thigh not found") end 
...
while event:Wait() do
    ...
    if humanoidRootPart.Velocity.Magnitude > 1 then -- walk
        ...
        leftShoulder.C0 = leftShoulder.C0:Lerp(cf(-1, 0.5, 0) * эйлер(-0,12217304763960307 * sin((синус + 25) * 0,05), -1,5882496193148399 + -0,17453292519943295 * sin((синус + 20) * 0,05), -2,0943951023931953), 0,2) 
        правоеПлечо.C0 = правоеПлечо.C0:Lerp(cf(1, 0,5, 0) * эйлер(-0,12217304763960307 * sin((синус + 25) * 0,05), 1,5707963267948966 + 0,17453292519943295 * sin((синус + 20) * 0,05), 2,0943951023931953), 0,2) 
        leftHip.C0 = leftHip.C0:Lerp(cf(-1, -1, 0) * euler(0, -1,5882496193148399, 0,3490658503988659 + -0,17453292519943295 * sin((синус + 30) * 0,05)), 0,2) 
    ...
    иначе 
        ...
        rootJoint.C0 = rootJoint.C0:Lerp(cf(0, -2 + 1 * sin(sine * 0,05), 0) * эйлер(-2,792526803190927 + -0,08726646259971647 * sin((синус + 10) * 0,05), 0,05235987755982989 * sin(синус * 0,025), -3,1590459461097367 + -0,08726646259971647 * sin(синус * 0,025)), 0,2) 
        левое плечо.C0 = левое плечо.C0:Lerp(cf(-1, 0,5, 0) * эйлер(-0,12217304763960307 * sin((синус + 25) * 0,05), -1,5882496193148399 + -0,17453292519943295 * sin((синус + 20) * 0,05), -1,2217304763960306), 0,2) 
        правое плечо.C0 = правое плечо.C0:Lerp(cf(1, 0,5, 0) * эйлер(-0,12217304763960307 * sin((синус + 25) * 0,05), 1,5707963267948966 + 0,17453292519943295 * sin((синус + 20) * 0,05), 1,2217304763960306), 0,2) 
...
---------------------------------------------- 
локальный selectedPlayer = nil 
локальный espEnabled = false 
локальный espBox = nil 
локальный bangActive = false 
локальный BGanimationTrack = nil 
локальная скорость = 10 
локальная функция findClosestMatch(input) 
    локальный inputLower = input:lower()
...
local function lockOn(targetInput) 
    ...
    if player then 
        ...
        notify("Locked On", "Игрок " .. player.Name .. " был заблокирован!", 5) 
    else 
        notify("Error", "Игрок либо ушел, либо его не существует!", 5) 
    end 
end 
...
local function unlockPlayer() 
    if selectedPlayer then 
        notify("Unlocked", "Игрок " .. selectedPlayer.Name .. " был разблокирован!", 5) 
        selectedPlayer = nil 
    else 
        notify("Error", "Ни один игрок не заблокирован!", 5) 
    end 
end 
...
local function getTorso(character) 
    return character:FindFirstChild("HumanoidRootPart") или character:FindFirstChild("Torso") 
end 
...
local function toggleFaceBang() 
    if bangActive then 
        ...
        if not selectedPlayer или not selectedPlayer.Character then 
            return OrionLib:MakeNotification({ 
                Name = "Face Bang", 
                Content = "Выбранный игрок недействителен!", 
                Time = 5, 
            }) 
        end 
        ...
        if not speaker.Character then 
            return OrionLib:MakeNotification({ 
                Name = "Face Bang", 
                Content = "Ваш персонаж не загружен!", 
                Time = 5, 
            }) 
        end 

        local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid") 
        если не гуманоид, то вернуть end 
            ...
            while not stopTeleport do
                if selectedPlayer and selectedPlayer.Character then
                    ...
                    if targetHumanoidRootPart and playerHumanoidRootPart then 
                        ...
                        notify("Ошибка", "Цель потеряна или недействительна!", 5) 
                        stopTeleport = true 
                        followActive = false 
                        return 
                    end 
                else 
                    notify("Ошибка", "Выбранный игрок недействителен!", 5) 
                    stopTeleport = true 
                    followActive = false 
                    return 
                end 
...
local function SkidFling(TargetPlayer) 
    local Character = game.Players.LocalPlayer.Character 
    local Humanoid = Character и Character:FindFirstChildOfClass("Humanoid") 
    local RootPart = Humanoid и Humanoid.RootPart 

    local TCharacter = TargetPlayer.Character 
    local THumanoid = TCharacter и TCharacter:FindFirstChildOfClass("Humanoid") 
    local TRootPart = THumanoid и THumanoid.RootPart 
    local THead = TCharacter и TCharacter:FindFirstChild("Head") 
    local Accessory = TCharacter и TCharacter:FindFirstChildOfClass("Accessory") 
    local Handle = Accessory и Accessory:FindFirstChild("Handle") 

    if Character и Humanoid и RootPart then 
        if RootPart.Velocity.Magnitude < 50 then 
            getgenv().OldPos = RootPart.CFrame 
        end 
        if THumanoid и THumanoid.Sit then 
            return notify("Произошла ошибка", "Цель сидит",5) 
        конец, 
        если THead, то
            ...
            Callback = function(val)
                if val ~= "" then
                    ...
                    elseif method == "3></-\\/\\/\\p|_3" then
                        ...
                        OrionLib:MakeNotification({
                            Name = "Ops.", 
                            Content = "Похоже, вы не выбрали метод!", 
                    ...
                    OrionLib:MakeNotification({ 
                        Name = "Oops.", 
                        Content = "Похоже, вы не ввели текст!", 
                        Image = "rbxassetid://4483345998", 
                        Time = 5 
                    }) 
                end 
        ...
        chatTab:AddToggle({ 
            ...
            Callback = function(val) 
                ...
                if val == true then 
                    OrionLib:MakeNotification({ 
                        Name = "Auto Bypass", 
                        Content = "Auto Bypass has включен!", 
                        Изображение = "rbxassetid://4483345998", 
                        Время = 5 
                    }) 
                else 
                    OrionLib:MakeNotification({ 
                        Имя = "Автоматический обход", 
                        Содержание = "Автоматический обход отключен!", 
                        Изображение = "rbxassetid://4483345998", 
                        Время = 5 
                    }) 
                end 
                    ...
                    if enter and autobypass then 
                        if textBox.Text ~= "" then 
                            ...
                            elseif method == "éxạmṕĺe" then 
                                ...
                                textBox.Текст = "" 
                            elseif метод == "ẹ̲х̲ạ̲ṃ̲р̲ḷ̲ẹ̲" then 
        ...
        chatTab:AddToggle({
            ...
            Callback = function(val)
                if webhook == "" then
                    ...
                    if val == true then 
                        OrionLib:MakeNotification({ 
                            Name = "Начато ведение журнала", 
                            Content = "Начато ведение журнала сообщений на указанном вебхуке!", 
                            Image = "rbxassetid://4483345998", 
                            Time = 5 
                        }) 
                    else 
                        OrionLib:MakeNotification({ 
                            Name = "Остановлено ведение журнала", 
                            Content = "Остановлено ведение журнала сообщений на указанном вебхуке!", 
                            Image = "rbxassetid://4483345998", 
                            Time = 5 
                        }) 
                    end 
